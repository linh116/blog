		...
		public function enqueue_admin_scripts( $hook ) {
			if ( isset( $_REQUEST['cp_admin_page_nonce'] ) && wp_verify_nonce( $_REQUEST['cp_admin_page_nonce'], 'cp_admin_page' ) ) {
				...
				|| ( isset( $_GET['style-view'] ) && 'new' === $_GET['style-view'] && CP_PLUS_SLUG . '_page_smile-info_bar-designer' === $hook ) ) {
					...
					if ( isset( $_GET['module'] ) && 'info_bar' === $_GET['module'] && ! wp_script_is( 'smile-info_bar-common-functions-js', 'enqueued' ) ) {
						wp_enqueue_script( 'smile-info_bar-common-functions-js', CP_PLUGIN_URL . 'modules/info_bar/assets/js/functions-common.js', array( 'jquery' ), CP_VERSION, true );
					}

					wp_localize_script(
						'smile-info_bar-common-functions-js',
						'cp',
						array(
					...
					wp_localize_script( 'smile-info_bar-common-functions-js', 'cp_active_theme', array( 'slug' => $theme_name ) );
					wp_localize_script( 'smile-info_bar-common-functions-js', 'smile_ajax', array( 'url' => admin_url( 'admin-ajax.php' ) ) );
				}
			}
		}
