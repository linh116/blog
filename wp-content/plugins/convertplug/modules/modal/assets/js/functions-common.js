		...
		if ( close_image_src == 'upload_img' ) {
			...
			if( close_img_default.indexOf('http') === -1 ) {
				...
				if( close_modal == "close_img" && close_img !== "" ) {
					...
					var img_data = {action:'cp_get_image',img_id:close_img,size:close_img_size,security_nonce: media_nonce};
					jQuery.ajax({
						url: smile_ajax.url,
						data: img_data,
						type: "POST",
...
jQuery(document).ready(function(){
	...
	jQuery("body").on("click", ".counter-overlay", function(e){ parent.setFocusElement('modal_desc_bg_color'); e.stopPropagation(); });
